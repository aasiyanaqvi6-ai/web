  ...
  return (
    <div className="relative min-h-screen w-full overflow-y-auto">
      <div ref={heroRef} className="relative min-h-screen w-full bg-transparent z-10">
        <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url('/public/bg.png')`, backgroundAttachment: 'fixed' }} />

        <div ref={portraitRef} className="absolute inset-0 flex items-center justify-center z-10" style={{ top: '-10%' }}>
          <div className="relative">
            <div className="w-[800px] h-[800px] overflow-hidden opacity-0 animate-fade-in-delayed" style={{ animationDelay: '0.3s', animationFillMode: 'forwards' }}>
              <img src="/public/me.png" alt="Portrait" className="w-full h-full object-cover grayscale contrast-110 brightness-90" />
              <div className="absolute inset-0 bg-gradient-to-t from-black/0 via-transparent to-transparent" />
            </div>
          </div>
        </div>

        <div ref={backgroundTextRef} className="absolute inset-0 flex items-center justify-center pointer-events-none" style={{ zIndex: 1, top: '65%' }}>
          <div className="text-[4rem] md:text-[10rem] lg:text-[15rem] font-bosenAlt text-black/30 select-none leading-none opacity-0 animate-fade-in-delayed" style={{ animationDelay: '0.1s', animationFillMode: 'forwards' }}>
            AAMIR NAQVI
          </div>
        </div>

        <div ref={mainTextRef} className="absolute inset-0 flex items-center justify-center pointer-events-none" style={{ top: '60%' }}>
          <div className="text-center z-10 px-6">
            <div className="text-2xl md:text-4xl lg:text-5xl font-bosenAlt tracking-tight text-white/80 leading-tight opacity-0 animate-fade-in-delayed" style={{ animationDelay: '0.8s', animationFillMode: 'forwards' }}>I EDIT</div>
            <div className="text-2xl md:text-3xl lg:text-4xl font-bosenAlt tracking-tight text-white/80 leading-tight mt-2 opacity-0 animate-fade-in-delayed" style={{ animationDelay: '1.1s', animationFillMode: 'forwards' }}>VISUALS THAT</div>
            <div className="text-2xl md:text-4xl lg:text-5xl font-bosenAlt tracking-tight text-white leading-tight mt-2 opacity-0 animate-fade-in-delayed" style={{ animationDelay: '1.4s', animationFillMode: 'forwards' }}>BUILD BRANDS</div>
          </div>
        </div>
        ...
        <div ref={triangleRef} className="absolute bottom-4 left-1/2 transform -translate-x-1/2 opacity-0 animate-fade-in-delayed z-30" style={{ animationDelay: '3.5s', animationFillMode: 'forwards' }}>
      ...
      <div ref={newSectionRef} className="relative z-20 bg-white min-h-screen" style={{ transform: 'translateY(100vh)' }}>
        <div className="container mx-auto px-6 py-20">
          <div className="max-w-4xl mx-auto text-center">
            <h2 className="text-4xl md:text-6xl font-bosenAlt text-gray-900 mb-8 opacity-0 animate-fade-in-delayed">PORTFOLIO</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mt-16">
              {[1, 2, 3, 4, 5, 6].map((item, index) => (
                <div key={item} className="bg-gray-100 aspect-square rounded-lg opacity-0 animate-fade-in-delayed hover:scale-105 transition-transform duration-300" style={{ animationDelay: `${0.6 + index * 0.1}s`, animationFillMode: 'forwards' }}>
